...
import {ethers} from '/assets/js/ethers-5.1.esm.min.js'
import {presaleAbi} from '/assets/contracts/presale_abi.js'
...
(function($){
    ...
    // presale contracts
    const contracts = {
        presale: '0xA51c1fc2f0D1a1b8494Ed1FE312d7C3a78Ed91C0'
    }

    let provider, myAddress, signer
    let presaleC
    ...
    const psEth = ethers.utils.parseEther

    async function initPresale() {
        presaleC = new ethers.Contract(contracts.presale, presaleAbi, provider);
        
        const filter = presaleC.filters.Presaled(myAddress, null)
        presaleC.on(filter, (addr, value, event) => {
            // The to will always be "address"
            console.log(`${ addr } send ${ value }.`);
            fetchPresaleData()
        });
        setTimeout(() => {
            fetchPresaleData()
    ...
    async function fetchPresaleData() {
        presaleData.isStart = await presaleC.isStart()
        presaleData.totalEther = await presaleC.totalEther()
        presaleData.myPresale = await presaleC.presaleList(myAddress)
        
        $('.total-eth.total').text((+fmtEth(presaleData.totalEther)).toFixed(2) + ' ETH')
        $('.total-eth.mysp').text((+fmtEth(presaleData.myPresale)).toFixed(2) + ' ETH')
        if (presaleData.myPresale.eq(0) || presaleData.totalEther.eq(0)) {
            $('.total-eth.reward').text('0 %')
        } else {
            $('.total-eth.reward').text(((+fmtEth(presaleData.myPresale)) / (+fmtEth(presaleData.totalEther)) * 100).toFixed(2) + ' %')
        }
    }


    async function doPresale() {

        await presaleC.connect(signer).doPresale({
            value: psEth($(".join-amount").val())
        })
    ...
    $(".default-btn.join").click(function () { 
        doPresale()
    });
